/**
 * Compact Porter stemmer (Martin Porter's algorithm). Used by the SNS Sentiment
 * demo to approximate the NLTK Snowball stemming the model was trained with.
 * Porter ≈ Snowball(english) for the vast majority of tokens; minor drift is
 * expected and disclosed in the case study.
 */
...
export function stem(w: string): string {
  ...
  if (re.test(w)) {
    ...
    stemmed = fp[1];
    re = s_v;
    if (re.test(stemmed)) w = stemmed + "i";
  }
  ...
  if (re.test(w)) {
    ...
    const suffix = fp[2];
    re = mgr0;
    if (re.test(stemmed)) w = stemmed + step2list[suffix];
  }
  ...
  if (re.test(w)) {
    ...
    const suffix = fp[2];
    re = mgr0;
    if (re.test(stemmed)) w = stemmed + step3list[suffix];
  }
  ...
  if (re.test(w)) {
    ...
    stemmed = fp[1];
    re = mgr1;
    if (re.test(stemmed)) w = stemmed;
  } else if (re2.test(w)) {
    const fp = re2.exec(w)!;
    stemmed = fp[1] + fp[2];
    re2 = mgr1;
    if (re2.test(stemmed)) w = stemmed;
  }
  ...
  if (re.test(w)) {
    ...
    if (re.test(stemmed) || (re2.test(stemmed) && !re3.test(stemmed))) w = stemmed;
  }
  re = /ll$/;
  re2 = mgr1;
